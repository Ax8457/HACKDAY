...
text= """
Welcome challengers, glad to have you on this twenty twenty six Hackday edition 
I really hope you will have a good time and learn a lot of new things
This challenge first looks like a very easy polyalphabetic challenge but if you never heard about Measure of Roughness I think you are likely to spend more time than you think trying to solve this one
The Measure of Roughness is based on the probability of occurence of each cipher character and compute a score. If you manage to get a measure of roughness of zero dot zero twenty eight you are on the right track insofar as the encrypted text is written in english
This method is simple and let you guess the lenght of the key
You have to divide the cipher text in k size groups meaning the key length is k
Then you you have to sum the probabilities elevated to square minus one divided by the number of characters of the ciphertext
If you find a suitable key length you are likely to get zero dot zero twenty eight as result
Then based on the size of the text and on the size the key you can make a frequential analysis to guess the key
Once done consider the hash sha two hundred fifty six of the key as your flag
I you read this it means you successfuly guessed the key lenght and manage to decrypt the cipher text
Congratulations and now submitt the key hashed as the flag using the right format and get some points as a reward
...
#key = ""
...
with open('mr_cipher.txt', 'w') as f:
	f.write(ciphertext)
